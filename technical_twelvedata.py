...
def analyze_daily_sma50(close: pd.Series) -> dict[str, Any]:
    ...
    if len(frame) < DAILY_TREND_DAYS:
        return {
            ...
            "Daily Recent SMA50 Change %": None,
    ...
    change_pct = ((now / start) - 1.0) * 100.0 if start else None
    recent_change_pct = ((now / recent_start) - 1.0) * 100.0 if recent_start else None
    ...
    recent_start = float(sma.iloc[-RECENT_TREND_DAYS])
    ...
    if not trend_ok:
        ...
        status = f"עבר: SMA50 עולה בשנה וב-3 החודשים האחרונים, המחיר מעל/נוגע בו, ו-{crossings} חציות מאושרות"
