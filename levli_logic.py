...
def ps_status(row: dict[str, Any]) -> str:
    ...
    if ps <= REASONABLE_PS:
        return "סביר"
    strong_growth = (
        (eps_growth is not None and eps_growth > 30) or
        (fwd_growth is not None and fwd_growth > 30)
    )
...
def mandatory_checks(row: dict[str, Any]) -> dict[str, bool]:
    return {
        "P/E > Forward P/E": (
            row.get("P/E") is not None and
            row.get("Forward P/E") is not None and
            row["P/E"] > row["Forward P/E"]
        ),
        "Quick Ratio > 1": (row.get("Quick Ratio") if row.get("Quick Ratio") is not None else -999) > MIN_QUICK_RATIO,
        "ROE ≥ 12%": (row.get("ROE %") if row.get("ROE %") is not None else -999) >= MIN_ROE,
        "ROIC ≥ 9%": (row.get("ROIC %") if row.get("ROIC %") is not None else -999) >= MIN_ROIC,
        "Gross Margin ≥ 38%": (row.get("Gross Margin %") if row.get("Gross Margin %") is not None else -999) >= MIN_GROSS_MARGIN,
        "Profit Margin ≥ 7%": (row.get("Profit Margin %") if row.get("Profit Margin %") is not None else -999) >= MIN_PROFIT_MARGIN,
    }
...
def screen_rows(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for row in rows:
        ...
        if all(checks.values()):
            ...
            failed.append(row)
    passed.sort(key=lambda r: (len(r.get("Levli Score","")), r.get("Ticker","")), reverse=True)
